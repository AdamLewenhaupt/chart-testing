(function() {
  $(function() {
    ...
    return $.getJSON('/data.json', function(data) {
      ...
      line = vis.append("svg:path").attr('d', lineGen(information["seb"])).attr('stroke', 'green').attr('stroke-width', 2).attr('fill', 'none');
      ...
      }).classed("point", true).attr('bank', "seb").attr("r", 5).call(drag);
    });
  });
