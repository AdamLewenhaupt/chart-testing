(function() {
  $(function() {
    return $.getJSON('/data.json', function(data) {
      var HEIGHT, MARGINS, WIDTH, drag, i, information, len, line, lineGen, precisionFormat, range, results, vis, xAxis, xScale, yAxis, yScale;
      ...
      for (i = 0, len = information.length; i < len; i++) {
        range = information[i];
        line = vis.append("svg:path").attr('d', lineGen(range.prediction)).classed('line', true);
        results.push(vis.selectAll().data(information['seb']).enter().append('circle').attr('cx', function(d) {
          return xScale(d.year);
        }).attr('cy', function(d) {
          return yScale(d.percentage);
