var DRAGGING, GRAPH_MARGINS, INACTIVE, LINECOLORS, RESULT_MARGINS, colorOffset, colorStep, createDrag, generateAxises, generateBackground, generateChart, generateGraphAxises, generateResult, i, j, onMouseLeaveLineOrDot, onMouseoverLineOrDot, randomResult, timeParser, timeParserMany;
...
$(function() {
  ...
  return $.getJSON('/data.json', function(data) {
    var headerItems, height, information, k, len, line, lineGen, lines, pointDrag, points, range, texts, vis, width, xScale, yScale;
    ...
    for (k = 0, len = information.length; k < len; k++) {
      range = information[k];
      line = vis.append("svg:path").attr('d', lineGen(range.prediction)).attr('range-index', range.range).classed('line', true).attr('stroke', LINECOLORS[range.range]).on('mouseover', function(d) {
        return onMouseoverLineOrDot.call(this, d, lines);
      }).on('mouseleave', function(d) {
        return onMouseLeaveLineOrDot.call(this, d, lines);
      });
