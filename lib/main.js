(function() {
  ...
  $(function() {
    return $.getJSON('/data.json', function(data) {
      var HEIGHT, MARGINS, WIDTH, colorCounter, drag, headerItems, i, information, len, line, lineGen, lines, precisionFormat, range, vis, xAxis, xScale, yAxis, yScale;
      ...
      for (i = 0, len = information.length; i < len; i++) {
        ...
        lines.push(line);
        vis.selectAll().data(range.prediction).enter().append('circle').attr('cx', function(d) {
          return xScale(d.year);
        }).attr('cy', function(d) {
          return yScale(d.percentage);
      ...
      headerItems.append('div').classed('graph-header-btn', true).style({
