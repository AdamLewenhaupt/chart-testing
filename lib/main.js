(function() {
  var DRAGGING, INACTIVE, LINECOLORS, setPosition;

  LINECOLORS = ["#00CCD6", "#FFD900", "#FB6648", "#FF2321", "#FF66CC", "#83BF17", "#E94C6F"];
  ...
  $(function() {
    return $.getJSON('/data.json', function(data) {
      var HEIGHT, MARGINS, WIDTH, colorCounter, drag, headerItems, height, i, information, len, line, lineGen, lines, onMouseoverLineOrDot, oneMouseleaveLineOrDot, points, precisionFormat, range, vis, width, xAxis, xScale, yAxis, yScale;
      ...
      yAxis = d3.svg.axis().scale(yScale).orient('left').innerTickSize(10).ticks(6).tickFormat(function(d) {
        return (precisionFormat(d)) + "%";
      });
      width = WIDTH - (MARGINS.left + MARGINS.right);
      height = HEIGHT - (MARGINS.top + MARGINS.bottom);
      setPosition(MARGINS.left, MARGINS.top, width, height, vis.append("svg:rect")).classed('graph-shadow', true).style("filter", "url(#dropshadow)");
