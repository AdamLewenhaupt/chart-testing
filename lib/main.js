var DRAGGING, INACTIVE, LINECOLORS, MARGINS, colorOffset, colorStep, generateAxises, generateBackground, generateChart, generateResult, i, j, timeParser, timeParserMany;
...
$(function() {
  return $.getJSON('/data.json', function(data) {
    ...
    vis.append("svg:rect").classed('graph-background', true).attr("fill", "url(#bars)").attr("filter", 'url(#dropshadow)').attr("x", MARGINS.left).attr("y", MARGINS.top).attr("width", width).attr("height", height).on("mousedown", function() {
...
generateAxises = function(vis, result, texts, width, height) {
  ...
  vis.append("svg:g").attr("transform", "translate(" + MARGINS.xaxis.right + "," + (height - (MARGINS.bottom - MARGINS.xaxis.down)) + ")").call(xAxis).classed('axis', true);
  vis.append("svg:g").attr("transform", "translate(" + (MARGINS.left - MARGINS.yaxis.left) + ", " + (MARGINS.yaxis.down + MARGINS.top) + ")").call(yAxis).classed('axis', true);
  return xScale;
};

generateBackground = function(vis, width, height) {
  return vis.append('svg:rect').attr("fill", "url(#bars-vertical)").attr("filter", "url(#dropshadow)").attr("width", width - MARGINS.left - MARGINS.right).attr("height", height - MARGINS.bottom - MARGINS.top).attr('x', MARGINS.left).attr('y', MARGINS.top);
};

generateChart = function(vis, result, width, height, xScale) {
  return vis.append('svg:g').selectAll('rect').data(result).enter().append('svg:rect').attr('height', (height - MARGINS.bottom - MARGINS.top) / result.length).attr('width', function(d) {
...
$(function() {
  var itr, ranges, result, sample, spawn;
