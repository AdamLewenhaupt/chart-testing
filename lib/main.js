...
$(function() {
  ...
  return $.getJSON('/data.json', function(data) {
    ...
    vis.append("svg:rect").classed('graph-background', true).attr("fill", "url(#bars)").attr("filter", 'url(#dropshadow)').attr("x", GRAPH_MARGINS.left).attr("y", GRAPH_MARGINS.top).attr("width", width).attr("height", height).on("mousedown", function() {
...
generateBackground = function(vis, width, height) {
  return vis.append('svg:rect').attr('id', 'background').attr("fill", "url(#bars-vertical)").attr("filter", "url(#dropshadow)").attr("width", width - RESULT_MARGINS.left - RESULT_MARGINS.right).attr("height", height - RESULT_MARGINS.bottom - RESULT_MARGINS.top).attr('x', RESULT_MARGINS.left).attr('y', RESULT_MARGINS.top);
};
