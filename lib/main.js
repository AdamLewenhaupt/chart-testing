var DRAGGING, INACTIVE, LINECOLORS, MARGINS, colorOffset, colorStep, generateAxises, generateBackground, generateChart, generateResult, i, j, randomResult, timeParser, timeParserMany;
...
generateBackground = function(vis, width, height) {
  return vis.append('svg:rect').attr('id', 'background').attr("fill", "url(#bars-vertical)").attr("filter", "url(#dropshadow)").attr("width", width - MARGINS.left - MARGINS.right).attr("height", height - MARGINS.bottom - MARGINS.top).attr('x', MARGINS.left).attr('y', MARGINS.top);
};
